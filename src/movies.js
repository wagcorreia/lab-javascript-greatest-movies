...
const stevenMovies = movies.filter(function (movie){
  ...
  const dramaMovies = stevenMovies.filter(function (movie){
...
function dramaMoviesScore(movies) {
  const dramaMoviesScore = movies.filter((movie)=> movie.genre.includes('Drama'));
    return scoresAverage(dramaMoviesScore);
...
function orderAlphabetically(movies) {
  let titles = movie.map((i) => i.title).sort();
